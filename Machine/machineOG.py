...
config.background_color = GRAY_D
...
class MachineAnimation2(MovingCameraScene):
    def construct(self):

        machine_rectangle = Rectangle(width=3, height=4).set_fill(GRAY_E, 1).set_z_index(-1)

        # Create gears and position them
        gears = SVGMobject(file_name='gears.svg').set_color(WHITE)
        gear1 = gears[:2]
        gear2 = gears[2:]
        ...
        # Progress bar
        progress_rec = Rectangle(height=0.25, width=2.5, stroke_width=2.5).next_to(machine_rectangle.get_top(), DOWN).set_z_index(10)
        ...
        # Lights
        dot1 = Dot(radius=DEFAULT_SMALL_DOT_RADIUS*2, fill_opacity=0.3, color=PURE_GREEN).set_stroke(width=1, color=WHITE)
        dot2 = Dot(radius=DEFAULT_SMALL_DOT_RADIUS*2, fill_opacity=0.3, color=PURE_GREEN).set_stroke(width=1, color=WHITE)
        dot3 = Dot(radius=DEFAULT_SMALL_DOT_RADIUS*2, fill_opacity=0.3, color=PURE_GREEN).set_stroke(width=1, color=WHITE)
        ...
        # Graphs
        square = Square(1).set_color(WHITE).next_to(machine_rectangle.get_bottom(), UP, buff=0.2).shift(RIGHT*0.8).scale(0.98).set_z_index(5)
        ...
        rec1 = Rectangle(width=0.12, height=1).set_fill(CHUTE_COLOR1, 1).set_stroke(width=0).next_to(machine_rectangle, LEFT, buff=0.02)
        rec2 = rec1.copy().set_color(CHUTE_COLOR2).next_to(rec1, LEFT, buff=0)
        ...
        sector1 = AnnularSector(inner_radius=0.7, outer_radius=1.1, angle=-PI/2, start_angle=PI+PI/4, color=CHUTE_COLOR1).next_to(chute1[-3], LEFT, buff=0).set_z_index(1)
        sector2 =  AnnularSector(inner_radius=1, outer_radius=1.1, angle=-PI/2, start_angle=PI+PI/4, color=CHUTE_COLOR2).next_to(sector1.get_left(), RIGHT, buff=0).set_z_index(2)
        ...
        dollar = SVGMobject(file_name='dollar.svg').set_color(WHITE).scale_to_fit_width(0.8)
        ...
        svgs = VGroup(dollar, clock, pin, tshirt).set_z_index(-5).arrange(DOWN, buff=0.4).to_edge(LEFT, buff=0.5)

        material = Tex('Material').set_color(WHITE)
        quantity = Tex('Quantity').set_color(WHITE)
        style = Tex('Style').set_color(WHITE)

        words = VGroup(material, quantity, style).arrange(DOWN, buff=1).to_edge(LEFT, buff=0.5)

        inputs = VGroup(dollar, style, tshirt, clock, quantity, pin, material).set_stroke(opacity=0).set_fill(opacity=0)

        # Manufacturer card
        rec_card = Rectangle(width=2, height=3, stroke_width=2).set_fill("#e8e7e1", 1).set_z_index(20).set_stroke(color=GRAY_E)
        ...
        self.add(machine_rectangle, gears, progress_rec, fill_rec, dots, square, chute1, chute2)
        self.add(inputs)
        self.wait(0.5)
        ...
        for n, i in enumerate(inputs):
            if n == 0:
                self.play(i.animate.set_stroke(opacity=2), run_time=0.4)
                self.play(
                ...
                self.play(i.animate.set_fill(opacity=2), run_time=0.4)
                self.play(
                    i.animate(rate_func=rate_functions.ease_in_expo).move_to(chute1).scale(0.4),
                    chute1.animate.move_to(chute1), 
                    run_time=0.3
        ...
        self.wait(0.5)
        self.play(card.animate.center().scale(3), run_time=2.5)
        self.wait()